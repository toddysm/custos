"""End-to-end stdin/stdout test for the Docker Hub plugin entry point.

Drives ``dockerhub_plugin.__main__.main`` with a fake ``sys.stdin`` and
...
from dockerhub_plugin.__main__ import main
...
def _request_envelope() -> dict[str, Any]:
    return {
        ...
        "connector": {
            ...
            "digest": "sha256:" + "b" * 64,
            "manifest": {"spec": {"target": {"endpoint": "https://registry-1.docker.io"}}},
        },
        "instance": {
            "workspaceId": "ws-1",
            "instanceId": "inst-1",
...
def test_main_success_path(fake_stdin: io.StringIO, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert response["result"]["endpoint"] == "https://registry-1.docker.io/v2/acme"
