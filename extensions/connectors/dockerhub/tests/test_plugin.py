"""Unit tests for the Docker Hub connector plugin hook handlers.
...
from dockerhub_plugin import handle
from dockerhub_plugin.plugin import PluginError
...
def _request(hook: str, *, hook_input: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "connector": {
            ...
            "version": "0.1.0",
            "imageRef": "ghcr.io/example/custos-dockerhub:0.1.0",
            "digest": "sha256:" + "a" * 64,
            "manifest": {
                "spec": {
                    "target": {
...
def test_manifest_declares_x_dapr_secret_and_dockerhub_target() -> None:
    manifest = json.loads((_PLUGIN_DIR / "connector-manifest.json").read_text())
    spec = manifest["spec"]
    assert manifest["metadata"]["type"] == "custos-dockerhub"
    assert spec["credentials"]["authenticationType"] == "x-dapr-secret"
    assert spec["target"]["endpoint"] == "https://registry-1.docker.io"
...
def test_bind_without_namespace_falls_back_to_bare_v2() -> None:
    ...
    assert result["endpoint"] == "https://registry-1.docker.io/v2"
...
def test_health_delegates_to_probe_and_wraps_result(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_probe(endpoint: str, *, verify_tls: bool = True, **_: Any) -> dict[str, Any]:
        ...
        return {
            "healthy": True,
            "detail": "ok",
            "registryEndpoint": "https://registry-1.docker.io/v2/",
            "tokenEndpoint": "https://auth.docker.io/token",
            "service": "registry.docker.io",
        }

    monkeypatch.setattr("dockerhub_plugin.plugin.probe.check_reachability", fake_probe)
    ...
    assert result["extras"]["tokenEndpoint"] == "https://auth.docker.io/token"
    assert captured == {"endpoint": "https://registry-1.docker.io", "verify_tls": True}


def test_health_unhealthy_when_probe_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "dockerhub_plugin.plugin.probe.check_reachability",
