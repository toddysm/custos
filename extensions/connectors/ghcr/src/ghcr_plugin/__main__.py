...
The runtime calls the plugin with one positional argument naming the hook
to invoke (``bind`` or ``health``) and a single JSON request envelope on
stdin. The plugin writes the response envelope on stdout. Any unexpected
failure is converted into the ``unknown-plugin-error`` envelope so the
runtime always sees a well-formed response and never has to interpret a
free-form stderr or non-zero exit code.
"""
