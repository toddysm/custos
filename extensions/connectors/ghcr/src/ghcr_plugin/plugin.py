"""Hook handlers for the out-of-the-box GHCR connector plugin.
...
* ``bind``   — return a :class:`ConnectorContext`-shaped JSON object so a
  workflow step can reach GHCR (GitHub Container Registry) through the
  connector sidecar. The context advertises ``tokenTypeHint: "basic"``:
  GHCR authenticates the Layer-2 token exchange with the GitHub PAT
  presented as HTTP Basic credentials, and the per-repository bearer is
  minted by the *consuming activity* (the plugin never mints it because
  the repository scope is unknown at bind time).
...
# GHCR's Layer-2 token endpoint and auth service, surfaced to the
# consuming activity via ``bind`` extras so it can run the PAT -> bearer
# exchange without re-deriving these constants.
_TOKEN_ENDPOINT: Final[str] = "https://ghcr.io/token"
_AUTH_SERVICE: Final[str] = "ghcr.io"

# This connector is fixed to GHCR. The manifest endpoint is
...
# to GHCR's registry endpoint over HTTPS.
_ALLOWED_REGISTRY_HOSTS: Final[frozenset[str]] = frozenset({"ghcr.io"})
...
def _bind(
    ...
    Derives the GHCR data-plane endpoint
    (``https://ghcr.io/v2/<namespace>``) from the manifest endpoint plus
    the per-instance ``repositoryNamespace``. ``handle`` is the binding
    the sidecar maps onto the leased PAT; ``extras`` carries the Layer-2
    token endpoint so the consuming activity can run the PAT ->
    per-repository bearer exchange. No token is minted here.
    """
    ...
    if capability not in _ADVERTISED_CAPABILITIES:
        raise PluginError(
            "invalid-response",
            f"ghcr plugin does not advertise capability {capability!r}; "
...
def _health(connector: dict[str, Any], instance: dict[str, Any]) -> dict[str, Any]:
    ...
    ``GET /v2/`` and verifies the GHCR Bearer challenge. The plugin has no
    resolved PAT, so this is the strongest signal it can produce without
    credentials.
    """
    target = _manifest_target(connector)
    endpoint = str(target.get("endpoint") or "")
    ...
    try:
        ...
        # Refuse to issue a live request at a non-GHCR host: return
