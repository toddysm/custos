"""Live, unauthenticated reachability probe for GHCR.

The ``health`` hook cannot authenticate. The plugin only ever receives a
*reference* to the GitHub Personal Access Token (the ``x-dapr-secret``
``authentication`` block), never the resolved secret, and ``bind`` hands
back a binding handle the sidecar maps onto the leased credential at the
data plane. So the probe verifies what it *can* without credentials: that
the registry is reachable, speaks the OCI distribution protocol, and
advertises the expected GHCR token endpoint.
...
``realm`` points at ``https://ghcr.io/token`` and whose ``service`` is
``ghcr.io``. Anonymous GHCR returns exactly that, so a matching challenge
is a strong liveness + protocol signal. The actual Layer-2 token exchange
(PAT -> per-repository bearer) is the consuming activity's job, not the
plugin's — see ``design/architecture/registry-credential-refresh.md``.
"""
...
_EXPECTED_REALM: str = "https://ghcr.io/token"
_EXPECTED_SERVICE: str = "ghcr.io"
...
def check_reachability(
    ...
    *,
    ...
    """Probe ``GET <endpoint>/v2/`` for the GHCR Bearer challenge.
    ...
    try:
        ...
        if healthy:
            ...
            detail = (
                "registry reachable but challenge does not match GHCR "
