"""Health endpoints (CS-IMPL-003 ``/readyz`` schema-gate behavior).
...
when :func:`custos_catalog.providers.verify_schema_revisions` raises
...
:func:`custos_catalog.providers.schema_gate_explainer`).
"""
...
from custos_catalog.providers import MigrationRequired, schema_gate_explainer
...
async def readyz(request: Request) -> JSONResponse:
    ...
    if isinstance(error, MigrationRequired):
        ...
        detail = "catalog-service has not finished startup"
    return JSONResponse(
        status_code=503,
