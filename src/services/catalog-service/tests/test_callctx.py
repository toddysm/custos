"""Tests for :mod:`custos_catalog.middleware.callctx` (CS-IMPL-004)."""
...
_VALID_CTX_HEADER = json.dumps(
    {
        ...
        "permissions": ["catalog:read", "catalog:publish"],
    },
)
...
def _build_app(*, authz_endpoint: str = "", environment: str = "development") -> FastAPI:
    ...
    @app.get("/publish")
    async def _publish(
        ctx: CallContext = Depends(require_permission("catalog:publish")),
    ) -> dict[str, str]:
        return {"workspace_id": ctx.workspace_id}

    # Force eager middleware instantiation so __init__-time guards
    # (e.g. the dev-shim production check) surface here rather than
...
def test_invalid_schema_header_returns_400() -> None:
    ...
    payload = json.dumps({"permissions": ["catalog:read"]})
...
def test_dev_shim_valid_header_populates_call_context(
    ...
    client = TestClient(_build_app())
    with caplog.at_level(logging.WARNING, logger="custos_catalog.middleware.callctx"):
        resp = client.get("/whoami", headers={CALLCTX_HEADER: _VALID_CTX_HEADER})
    assert resp.status_code == 200
    assert resp.json() == {
        "workspace_id": "ws_demo",
        "principal_id": "user_alice",
        "permissions": ["catalog:publish", "catalog:read"],
    }
...
def test_dev_shim_emits_audit_event(caplog: pytest.LogCaptureFixture) -> None:
    client = TestClient(_build_app())
    with caplog.at_level(logging.INFO, logger="custos_catalog.audit"):
...
def test_require_permission_grants_when_present() -> None:
    client = TestClient(_build_app())
    resp = client.get("/publish", headers={CALLCTX_HEADER: _VALID_CTX_HEADER})
    assert resp.status_code == 200
