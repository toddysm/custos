...
- **Streaming digest computation** — SHA256 computed on-the-fly as bytes arrive (O(1) memory)
- **Temp file spooling** — content spooled to temp file while digesting, then uploaded to S3 (O(n) disk, O(1) RAM)
