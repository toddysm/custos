...
| [`custosctl` CLI](custosctl.md) | Local & remote dev/test CLI: `up`/`down`/`status`/`doctor` lifecycle, `connector`/`activity` register+list, `workflow` apply/run/status, `seed-ootb`, and the `e2e` smoke; configuration and target model |
