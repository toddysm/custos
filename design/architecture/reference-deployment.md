...
- [x] TODO-005: ~~Define the Grafana dashboard bundle contents (per-component dashboards, audit-event dashboard, drainer-lag dashboard).~~ **Resolved 2026-06-07:** three sidecar-discoverable dashboards ship under `deploy/helm/custos/dashboards/` and render as labeled ConfigMaps (`grafana_dashboard: "1"`) via `templates/grafana-dashboards.yaml` — *Custos — Components Overview* (gateway/auth/catalog/connector/trigger/workflow/ARM request, latency, and error panels), *Custos — Audit Events* (emit failures, retention, alert dispatch, exporter-config status), and *Custos — Audit Drainer Lag* (`custos_obs_audit_outbox_lag_rows`). (added 2026-05-17)
...
- [x] TODO-007: ~~Document the upgrade-with-breaking-schema runbook (maintenance window, read-only fallback for the migration window, rollback procedure).~~ **Resolved 2026-06-07:** see [docs/operations/upgrade-with-breaking-schema-runbook.md](../../docs/operations/upgrade-with-breaking-schema-runbook.md) — covers window sizing from a production-data rehearsal, Gateway-edge read-only fallback (write-path scale-to-zero / 503 on mutating verbs), the forward-only migration point-of-no-return, and the two rollback cases (Job-failed vs migration-committed restore-from-backup). (added 2026-05-17)
