...
- [ ] TODO-009: ~~Finalize platform event taxonomy mapping for activity lifecycle events with Observability.~~ **Resolved 2026-06-04** by the unified platform event taxonomy (Trigger Service TODO-001, #18): ARM emits the `activity.*` lifecycle strings verbatim from the shared closed registry, so cross-cutting events like `workflow.completed`, `step.completed`, `activity.failed` carry one canonical name from emission through trigger matching, audit storage, and consumer dashboards. See trigger-service `changes/2026-06-04-007-event-taxonomy.md` (INCON-013, #38).
