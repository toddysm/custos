...
- [x] WF-IMPL-103 (#542): `DaprTriggerServiceClient` — Dapr Service-Invocation adapter (depends on #540).
...
- [x] WF-IMPL-106 (#545): Cancel-run + terminal cancellation of open subscriptions (depends on #543).
...
- [x] WF-IMPL-108 (#547): `providers.py` wiring + Configuration knobs + registration (depends on #544, #545, #546).
- [x] WF-IMPL-109 (#548): TTL-expiry periodic mirror sweep (depends on #541, #547).
...
## Implementation — Durable Wiring

Ninth sub-module: **Durable Wiring**, packaged inside the service host `src/services/workflow-service/`. Replaces the deployed-build stubs in `providers.py` with production adapters so the service runs end-to-end in a cluster: a `DaprCatalogClient` (`GetWorkflowVersion` over Dapr Service Invocation) replaces the raising `_NotConfiguredCatalogClient`; a lifespan-owned `custos_pg` `MetadataStoreProvider` (new `WF_METADATA_STORE` knob) backs a durable Run store and a durable `IdempotencyLedger`, replacing the in-memory defaults. The durable backend (`custos_state.run` / `step_attempt` / `idempotency_record` + migrations) and the Catalog Internal RPC already exist, so this is wiring against established `DaprTriggerServiceClient` / `_dapr_invoke` and `CAT_*_STORE` precedents. Plan: [`implementation-plan-durable-wiring.md`](implementation-plan-durable-wiring.md).
...
- [x] WF-IMPL-085 (#508): Deterministic child-instance-id + iteration-key derivation.
- [x] WF-IMPL-086 (#509): Sub-orchestration error taxonomy additions.
- [x] WF-IMPL-087 (#510): `approval:` step kind + model + compiler primitive tagging.

### Phase B — Child workflow + dynamic loop

- [x] WF-IMPL-088 (#511): Child sub-workflow orchestrator function (depends on #507, #508).
- [x] WF-IMPL-089 (#512): Loop fan-out — `forEach` spawn + `when_all` + merge (depends on #507, #508, #509, #511).
- [x] WF-IMPL-090 (#513): `where:` pre-filter + loop edge cases (depends on #512).
...
- [x] WF-IMPL-098 (#521): Developer documentation — `docs/developers/workflow-sub-orchestration.md` (depends on #520).
