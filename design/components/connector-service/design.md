...
## Webhook Signature Verification (push mode)

For `push`-mode connector instances that receive webhooks, the **signing/verification material is part of the instance credential model** (owned here, per `ConnectorInstance`, resolved through the Secret Bridge / an identity resolver such as `x-dapr-secret`). The Trigger Service's Generic Webhook Receiver is the HTTP entry point but **delegates verification to the Connector Service**: it calls a verification seam with the raw body + headers for a `connectorInstanceId`; the Connector Service resolves the instance secret and runs the connector plugin's vendor-specific verifier (e.g. GitHub `X-Hub-Signature-256`, Slack signing secret, generic HMAC). Raw signing secrets never leave the Connector Service boundary; the Trigger Service sees only the pass/fail outcome. Rotation is an ordinary Connector-Service credential operation on the instance secret, with no Trigger Service change. See Trigger Service design § Webhook Signature Verification (#23).
...
| 2026-08-29 | TS-TODO-006: webhook signing/verification material is owned here, per `ConnectorInstance` (part of the credential model); the Connector Service verifies inbound `push`-mode webhook signatures on the Trigger Service's behalf via a verification seam, so raw secrets never leave this boundary; added § Webhook Signature Verification (push mode) | #23 |
